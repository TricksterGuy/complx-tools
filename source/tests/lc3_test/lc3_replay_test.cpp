...
BOOST_FIXTURE_TEST_CASE(ReplayTestRejectInvalidFilename, LC3ReplayTest)
{
    ...
    std::stringstream file(asm_file);
    std::stringstream input;
    BOOST_CHECK_THROW(lc3_setup_replay(state, "b", file, replay, input), LC3ReplayStringException);
}
...
BOOST_FIXTURE_TEST_CASE(ReplayTestRejectBadCompression, LC3ReplayTest)
{
    ...
    std::stringstream file(asm_file);
    std::stringstream input;
    BOOST_CHECK_THROW(lc3_setup_replay(state, "test.asm", file, replay, input), LC3ReplayStringException);
}